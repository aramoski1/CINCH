...
export function CreateScreen({
    ...
    const id = window.setTimeout(() => {
      void api.parse(trimmed).then((body) => {
        ...
        if (spec.schedule?.deadline_at) {
          setWhen(
            new Date(spec.schedule.deadline_at).toLocaleString(undefined, {
              weekday: "short",
              month: "short",
              day: "numeric",
              hour: "numeric",
  ...
  const canLock = Boolean(picked) && utterance.trim().length > 3 && !busy;

  return (
    <section>
      <p className="eyebrow">New promise</p>
      <h1 className="hero">Who in your network?</h1>
      <p className="lede">Pick the friend who collects if you miss. Photo proof or you lose.</p>
      ...
      {picked ? (
        <>
          ...
          {title ? (
            ...
            <p className="muted mt-3">Say it like a text. We'll catch the time.</p>
          )}

          <p className="eyebrow mt-4">On the line</p>
          ...
          <p className="muted mt-3">
            Prove it with a photo before time's up. Miss the picture and {formatStake(stake * 100)} goes to {friendName}.
          </p>
